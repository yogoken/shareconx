class TweetsController < ApplicationController
  ...
  def show
    @tweet = Tweet.find(1)
    @tweet_like = current_user.tweet_likes.find_by(tweet_id: 1)
    @tweet_likes = TweetLike.where(tweet_id: 1)

    @tweet_bookmark = current_user.bookmarks.find_by(tweet_id: 1)

    @least_comments = Comment.where(tweet_id: 1).order('id asc')

    @comment_likes = CommentLike.where(comment_id: params[:comment_id])

    @comments = Comment.where(tweet_id: 1).includes(:tweet)
    @replies = Reply.where(tweet_id: 1)
